...
public partial struct TowerPlacementSystem : ISystem
{
    ...
    public void OnCreate(ref SystemState state)
    {
        ...
        state.RequireForUpdate<BuildingPrefab>();
    }
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        DynamicBuffer<BuildingPrefab> towers = SystemAPI.GetSingletonBuffer<BuildingPrefab>();
        ...
        foreach (var input in SystemAPI.Query<DynamicBuffer<TowerPlacementInput>>())
        {
            foreach(TowerPlacementInput placementInput in input)
            {
                ...
                if (physicsWorld.OverlapSphere(towerPosition, 0.4f, ref distances,
                        anythingButTerrain))
